    ...
    while retries < max_retries:
        try:
            ...
            if data['items'] and data['items'][0].get('mostReplayed'):
                ...
                # Previous code: Find the most replayed part and its time frame
                # most_replayed = max(
                #     heat_markers, key=lambda x: x['heatMarkerRenderer']['heatMarkerIntensityScoreNormalized'])
                # start_time_ms = most_replayed['heatMarkerRenderer']['timeRangeStartMillis']
                # end_time_ms = start_time_ms + \
                #     most_replayed['heatMarkerRenderer']['markerDurationMillis']

                # New code: Find the most replayed part by intensity score and determine the time frame
    ...
    while retries < max_retries:
        try:
            ...
            download_path = stream.download(
                filename=f"{youtube_id}/full_video.mp4")
            ffmpeg_extract_subclip(
                download_path, start_time, end_time, targetname=f"{youtube_id}/{output_mp4}")
...
def main():
    ...
    output_mp4 = input(
        "Enter the desired output filename (e.g. highlight_clip.mp4): ")
    ...
    try:
        ...
        with open(f"{youtube_id}/{youtube_id}-highlight-transcription.txt", "w") as f:
            ...
            print(f"Transcription saved to " + youtube_id + "/" +
